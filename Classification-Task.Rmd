...
svm.test <- function(gamma, cost) {
  ...
  for(i in 1:num_imp) {
    print(i)
    svm.models[[i]] <- svm(factor(SeriousDlqin2yrs) ~ ., data = reduce(complete(imputed_data_train, i)), 
                           gamma=gamma, 
                           kernel="linear",
                           cost=cost,
                           probability = TRUE)
  ...
  predictions <- lapply(1:num_imp, function(m) {
    predict(svm.models[[m]], newdata = complete(imputed_data_test, m), probability = TRUE)
  })
...
```{r}
dim(complete(imputed_data_train, 1))
dim(complete(imputed_data_test, 1))
```
