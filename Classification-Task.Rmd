...
xgboost_objective <- function(max_depth, eta) {
  n <- nrow(data)
  splits <- c(seq(1,n, n/10),n+1)
  jumps <- n/10
  fitted <- c()
  for (i in 1:10) {
    ...
    fit <- xgboost(data = train_matrix, params = params, nrounds = 10, verbose = 0)
...
bayesian_optimized_rf <- BayesianOptimization(FUN=xgboost_objective, bounds=list(max_depth=c(2, 5), eta=c(0.001,0.1)), init_points=10, n_iter=20)
```
