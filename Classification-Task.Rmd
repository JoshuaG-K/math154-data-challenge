...
svm.cv1 <- function(gamma, cost) {
  ...
  for (i in 1:10) {
    ...
    fit <- svm(y~., data=train.data, kernel="radial", gamma=gamma, cost=cost)
...
bayesian_optimized_svm <- BayesianOptimization(svm.cv1, bounds=list(gamma=c(0,0.001), cost=c(1000,1500)), init_points=20, n_iter=30 )
```
...
print(best_cost)
print(best_gamma)
best_svm <- svm(factor(SeriousDlqin2yrs) ~ ., data=data.train, kernel="radial", cost = best_cost, gamma = best_gamma)
