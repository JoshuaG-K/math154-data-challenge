...
svm.cv1 <- function(gamma, cost) {
  ...
  for (i in 1:10) {
    ...
    fitted <- c(fitted, attributes(predict(fit, test.data, decision.values=TRUE, probability=TRUE))$decision)
    # print(y)
    # print(length(y))
    # print(length(fitted))
...
bayesian_optimized_svm <- BayesianOptimization(svm.cv1, bounds=list(gamma=c(0,0.0001), cost=c(1000,1500)), init_points=20, n_iter=20 )
```
...
```{r}
predictions <- predict(best_svm, data.test, probability=TRUE)
