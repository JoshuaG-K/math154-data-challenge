...
```{r}
data <- data_imputed_no_labels
y <- y_imputed #droplevels(iris[51:150,5])
b <- sample(1:100) 
data <- data[b,]
y <- y[b]
...
svm.cv1 <- function(gamma, cost) {
  ...
  for (i in 1:10) {
    ...
    for(i in seq_along(imputed_data_test)) {
        svm_models[[i]] <- svm( SeriousDlqin2yrs ~ . - X, data = imputed_data_sets[[i]], 
                                gamma=gamma, 
                                cost=cost)
    }
    
    # fit <- svm(y~., data=train.data, gamma=gamma, cost=cost)
    fit <- with(imputed_data_train$imp, svm(SeriousDlqin2yrs~.-X, gamma=gamma, cost=cost))

    predictions <- lapply(1:num_imp, function(m) {
      # Assuming 'fit' is the list of models you trained
      predict(fit[[m]], newdata = imputed_data_test[imputed_data_test$.imp == m,])
    })
    
    # For classification (binary as an example):
    # Convert predictions to binary (if needed)
    binaryPredictions <- lapply(predictions, function(x) ifelse(x > 0.5, 1, 0))
    # Calculate mode for each observation
  ...
  return (0)#(list(Score=roc(y,fitted)$auc, Pred=0))
}
test <- svm.cv1(0.377, 508)
# bayesian_optimized_svm <- BayesianOptimization(svm.cv1, bounds=list(gamma=c(0,2), cost=c(0,1000)), init_points=10, n_iter=20 )
```
