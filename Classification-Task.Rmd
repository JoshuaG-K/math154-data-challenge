...
xgboost_objective <- function(eta) {
  ...
  for (i in 1:10) {
    ...
    fit <- xgboost(data = train_matrix, params = params, nrounds = 100, verbose = 0)
    pred <- predict(fit, test_matrix, type="response")
    probabilities <- pred
...
# t <- xgboost_objective(0.1)
bayesian_optimized_rf <- BayesianOptimization(FUN=xgboost_objective, bounds=list(eta=c(0.001,0.1)), init_points=500, n_iter=20)
```
...
for(i in 1:num_imp) {
    ...
    imputed_train_no_label <- imputed_train[,-which(names(imputed_train) == column_to_remove)]
    imputed_train_label <- imputed_train[,which(names(imputed_train) == column_to_remove)]
    
    imputed_test <- complete(imputed_data_test, i)
    imputed_test_no_label <- imputed_train[,-which(names(imputed_test) == column_to_remove)]

    # Subset the training data
    subset_train <- imputed_train[1:(nrow(imputed_train) * subset_size), ]
    subset_train_no_label <- imputed_train[1:(nrow(imputed_train_no_label) * subset_size), ]
    subset_train_label <- imputed_train[1:(nrow(imputed_train_label) * subset_size), ]
    
    train_matrix <- xgb.DMatrix(data=as.matrix(subset_train_no_label), label=as.numeric(as.character(subset_train_label)))
    test_matrix <- xgb.DMatrix(data=as.matrix(imputed_test_no_label))
    ...
    rf_prediction <- predict(fit, test_matrix, type="response")
    
    # Extract probabilities
    prob_values <- rf_prediction # attr(rf_prediction, "probabilities")
    # print(prob_values)
